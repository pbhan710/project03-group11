var TimeKnots = {
  draw: function(id, events, options){
    var cfg = {
      ...
      addNow: false,
      seriesColor: d3v2.scale.category20,
      dateDimension: true
    };
    ...
    var svg = d3v2.select(id).append('svg').attr("width", cfg.width).attr("height", cfg.height);
    //Calculate times in terms of timestamps
    if(!cfg.dateDimension){
      var timestamps = events.map(function(d){return  d.value});//new Date(d.date).getTime()});
      var maxValue = d3v2.max(timestamps);
      var minValue = d3v2.min(timestamps);
    }else{
      var timestamps = events.map(function(d){return  Date.parse(d.date);});//new Date(d.date).getTime()});
      var maxValue = d3v2.max(timestamps);
      var minValue = d3v2.min(timestamps);
    }
    var margin = (d3v2.max(events.map(function(d){return d.radius})) || cfg.radius)*1.5+cfg.lineWidth;
    ...
    .attr("cx", function(d){
      ...
      if(cfg.dateDimension){
        var format = d3v2.time.format(cfg.dateFormat);
    ...
    if(cfg.showLabels != false){
      if(cfg.dateDimension){
        var format = d3v2.time.format(cfg.labelFormat);
      ...
      svg.append("text")
         ...
         .attr("x", function(d){if(cfg.horizontalLayout){return d3v2.max([0, (margin-this.getBBox().width/2)])} return Math.floor(this.getBBox().width/2)})
      ...
      svg.append("text")
         ...
         .attr("x", function(d){if(cfg.horizontalLayout){return  cfg.width -  d3v2.max([this.getBBox().width, (margin+this.getBBox().width/2)])} return Math.floor(this.getBBox().width/2)})
    ...
    return tip.style("top", (d3v2.event.pageY-tipPixels-margin)+"px").style("left",(d3v2.event.pageX+20)+"px");})
